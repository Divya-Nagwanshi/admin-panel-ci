...
class UserControl extends CI_Controller {
    ...
    public function register()
    {
        ...
        $this->form_validation->set_rules('name', ' Full Name', 'trim|required|alpha');
        $this->form_validation->set_rules('email', 'Email', 'required|valid_email|is_unique[users.email]');
        $this->form_validation->set_rules('password', 'Password', 'required|min_length[6]');
        $this->form_validation->set_rules('r_password', 'Re Type', 'required|min_length[6]');
        $this->form_validation->set_rules('mobile','Mobile No ',array('trim','required','min_length[10]', 'max_length[10]','regex_match[/^[6-9]\d{9}$/]','is_unique[users.mobile]'));
